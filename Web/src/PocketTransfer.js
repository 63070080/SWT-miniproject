...
function PocketTransfer() {
    ...
    const transfer = async () => {
        if (selectOp1 == "Main") {
            ...
            const response = await fetch('http://server/m2p/' + user._id, {
                method: "PUT",
            ...
            const response = await fetch('http://server/p2m/' + user._id, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(send)
            });
            ...
            const response = await fetch('http://server/p2p/' + user._id, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(send)
            });
