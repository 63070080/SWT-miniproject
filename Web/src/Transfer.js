...
function Transfer(){
  ...
  useEffect(()=>{
    ...
    const transfer = async() => {
        if(selectOp1 == "Main"){
            const response = await fetch('http://52.23.159.62:8085/transfer/'+user._id+"/"+cash+"?target="+target, {
              method: "PUT"
            });
            ...
            const response = await fetch('http://52.23.159.62:8085/transfer/'+user._id+"/"+cash+"?target="+target+"&pocketid="+selectOp1._id, {
              method: "PUT"
            });
            const result = await response.json();
            console.log(result)
