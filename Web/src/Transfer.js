  ...
  useEffect(()=>{
    ...
    async function genUser(){
      if(localStorage.getItem('userId')){
        const response2 = await fetch('http://3.90.8.217:8085/users/'+localStorage.getItem('userId'));
        ...
        try {
          ...
          const response = await fetch('http://3.90.8.217:8085/create_user', {
            method: "POST",
          ...
          console.log(json)
          const response2 = await fetch('http://3.90.8.217:8085/users/'+json.id);
          const result = await response2.json();
    ...
    const transfer = async() => {
        if(selectOp1 == "Main"){
            const response = await fetch('http://3.90.8.217:8085/transfer/'+user._id+"/"+cash+"?target="+target, {
              method: "PUT"
            });
            ...
            const response = await fetch('http://3.90.8.217:8085/transfer/'+user._id+"/"+cash+"?target="+target+"&pocketid="+selectOp1._id, {
              method: "PUT"
            });
            const result = await response.json();
            console.log(result)
